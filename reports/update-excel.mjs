...
function complexityStyle(complexity) {
  switch (complexity) {
    case "Basic":
      return { fill: solidFill(C.BASIC_BG), font: { bold: true, size: 10, color: C.BASIC_FG } };
    case "Medium":
      return { fill: solidFill(C.MEDIUM_BG), font: { bold: true, size: 10, color: C.MEDIUM_FG } };
    case "High":
      return { fill: solidFill(C.HIGH_BG), font: { bold: true, size: 10, color: C.HIGH_FG } };
    case "Enterprise":
      return { fill: solidFill(C.ENT_BG), font: { bold: true, size: 10, color: C.ENT_FG } };
    default:
      return {};
  }
}
...
function styleMainDataRow(row, rowIndex) {
  ...
  c1.fill = mktBg;
  c1.font = { bold: true, size: 10, color: C.MKT_FG };
  c1.alignment = { vertical: "middle", wrapText: true };
  c1.border = thinBorder();
  // Col 2: Industries
  const c2 = row.getCell(2);
  c2.fill = mktBg;
  c2.font = { size: 9.5, color: { argb: "FF544E45" } };
  ...
  c3.fill = cs.fill || mktBg;
  c3.font = cs.font || { size: 10 };
  c3.alignment = { vertical: "middle", horizontal: "center" };
  c3.border = thinBorder();
  // Col 4: Market Price
  const c4 = row.getCell(4);
  c4.fill = mktBg;
  c4.font = { bold: true, size: 10, color: C.MKT_FG };
  c4.alignment = { vertical: "middle" };
  c4.border = thinBorder();
  // Col 5: Market Delivery
  const c5 = row.getCell(5);
  c5.fill = mktBg;
  c5.font = { size: 10, color: C.MKT_FG };
  c5.alignment = { vertical: "middle" };
  c5.border = thinBorder();
  // Col 6: Market Maint
  const c6 = row.getCell(6);
  c6.fill = mktBg;
  c6.font = { size: 9.5, color: C.MKT_FG };
  c6.alignment = { vertical: "middle" };
  c6.border = thinBorder();
  // Col 7: ORVION Price → terracotta
  const c7 = row.getCell(7);
  c7.fill = solidFill(C.ORV_PRICE_BG);
  c7.font = { bold: true, size: 11, color: C.ORV_PRICE_FG };
  ...
  c8.fill = solidFill(C.ORV_DEL_BG);
  c8.font = { bold: true, size: 10.5, color: C.ORV_DEL_FG };
  c8.alignment = { vertical: "middle", horizontal: "center" };
  c8.border = thinBorder();
  // Col 9: ORVION Maint → teal
  const c9 = row.getCell(9);
  c9.fill = solidFill(C.ORV_MAINT_BG);
  c9.font = { bold: true, size: 10, color: C.ORV_MAINT_FG };
...
function styleColHdrRow(row) {
  row.eachCell((cell) => {
    ...
    cell.border = {
      bottom: { style: "medium", color: { argb: "FFC75B3A" } },
...
// ──── REMOVE MAINTENANCE SHEET (05) ──────────────────────────────────────────
{
  ...
  if (ws) {
    ...
    console.log("✓ Removed: 05 — Maintenance");
  }
}
  ...
  if (ws) {
    ...
    ws.eachRow((row) => {
      ...
      if (isColHdr) {
        ...
        c1.border = thinBorder();
        const c2 = row.getCell(2);
        c2.fill = solidFill(C.ORV_PRICE_BG);
        c2.font = { bold: true, size: 11, color: C.ORV_PRICE_FG };
        c2.alignment = { vertical: "middle", horizontal: "center" };
        c2.border = thinBorder();
        const c3 = row.getCell(3);
        c3.fill = mktBg;
        c3.font = { size: 10, color: { argb: "FF544E45" } };
