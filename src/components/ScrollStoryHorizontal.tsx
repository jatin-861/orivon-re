...
export function ScrollStoryHorizontal({ projects }: ScrollStoryHorizontalProps) {
  ...
  // Desktop AND phone both get the cinematic auto-scroll. The phone jiggle that
  // showed up in earlier versions was NOT inherent to running GSAP on touch — it
  // was two separate bugs now fixed:
  //   1. scrub used a duration value, so GSAP's easing lagged on top of Lenis's
  //      own easing → two springs chasing each other = rubber-band jiggle.
  //      scrub: true tracks the (already-smoothed) scroll position 1:1.
  //   2. the mobile address bar collapsing mid-scroll re-ran the pin → vertical
  //      "up-down" judder. ScrollTrigger.config({ ignoreMobileResize: true })
  //      (set in LenisProvider) ignores that, and a stable 100svh panel height
  //      (small-viewport unit — does not grow/shrink with the address bar) means
  //      the pin geometry never changes mid-scroll.
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      const getDistance = () => scrollSection.scrollWidth - window.innerWidth;
      ...
      const horizontalTween = gsap.to(scrollSection, {
        ...
        scrollTrigger: {
          ...
          pin: true,
          pinType: "transform",
          anticipatePin: 1, // smooths the hand-off into the pin (less first-frame jump on mobile)
          scrub: true, // 1:1 with scroll position — no second easing layer (see note above)
          start: "top top",
          end: () => `+=${getDistance() * 1.5}`,
          invalidateOnRefresh: true, // recompute distances on resize/refresh instead of mid-scroll
  ...
  return (
    ...
    >
      ...
      {/* Track: a w-max flex row that GSAP translates horizontally as you scroll
          vertically — no native horizontal overflow (so no scrollbar) on any
          device. touch-pan-y tells the browser touches here drive vertical page
          scroll (which is what powers the GSAP motion), removing any gesture-axis
          ambiguity that could cause a sideways tug. */}
      <div
        ref={scrollRef}
        className="flex h-full w-max items-center relative z-10 touch-pan-y"
      >
        {/* Intro Slide */}
        <div
          data-panel
            ...
            >
              ...
              >
                ...
                <div className="md:col-span-7 h-40 md:h-full flex items-center justify-center relative overflow-hidden rounded-2xl">
                  ...
                  >
                    <div className="p-4 md:p-6 h-full flex flex-col justify-between" data-parallax-inner>
                      {/* Window Controls */}
